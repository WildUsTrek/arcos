...
Add-Check "dynamic-hand-count-layout" ($cardText -match "visibleHandCount" -and $cardText -match "ownerHandCount" -and $cardText -match "getVisibleHandCount" -and $cardText -match "h\$\{visibleHandCount\}" -and $cardText -match "card-pos-\$\{posMode\}" -and $cardHandLayoutText -match "maxCardsInHand \+ 1" -and $cardHandLayoutText -match "card\.position \+ 1" -and $zoneCardsInnerText -match "handLayoutCounts" -and $zoneCardsInnerText -match "getOwnerHandLayoutCounts" -and $zoneCardsInnerText -match "ownerHandCount" -and $cardPosStyleText -match "handCount <= maxCardsInHand \+ 1" -and $cardPosStyleText -match "card-pos-\$\{mode\}" -and $cardPosStyleText -match "--cardwidth") "Card layout must generate position and size classes from the real rendered owner hand count, including transition card frames"
