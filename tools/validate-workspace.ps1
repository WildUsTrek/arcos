...
Add-Check "campaign-battle-intro-no-global-skip" ($campaignIntroText -notmatch "role=`"presentation`"" -and $campaignIntroText -notmatch "<div\s+className=\{styles\.overlay\}\s+onClick" -and $campaignIntroText -match "Inizia battaglia") "Campaign battle intro must not be globally skippable before rules are readable"
