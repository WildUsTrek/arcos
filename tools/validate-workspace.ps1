...
Add-Check "dynamic-hand-count-layout" ($cardText -match "visibleHandCount" -and $cardText -match "maxCardsInHand \+ 1" -and $cardText -match "h\$\{visibleHandCount\}" -and $cardText -match "card-pos-\$\{posMode\}" -and $cardPosStyleText -match "handCount <= maxCardsInHand \+ 1" -and $cardPosStyleText -match "card-pos-\$\{mode\}" -and $cardPosStyleText -match "--cardwidth") "Card layout must generate position and size classes for every real visible hand count, including transition card frames"
