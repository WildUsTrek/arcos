...
Add-Check "campaign-battle-intro-no-global-skip" ($campaignIntroText -notmatch "role=`"presentation`"" -and $campaignIntroText -match "Inizia battaglia" -and $campaignIntroText.IndexOf("setHidden(true)") -lt $campaignIntroText.IndexOf("type: UPDATE_SETTINGS_INIT") -and $campaignIntroStylesText -match "pointer-events: auto") "Campaign battle intro must not be globally skippable before rules are readable"
...
Add-Check "campaign-battle-intro-landscape-gated" ($windowListText -match "!pref &&\s*!landscape" -and $windowListText -match "!battleIntroAcknowledged" -and $campaignIntroStylesText -match "aspect-ratio: 16 / 9" -and $campaignIntroStylesText -match "z-index: 130") "Campaign battle intro must respect landscape-first gameplay and must not replay after landscape notice remounts"
...
Add-Check "mobile-visual-viewport" ($gameSizeProviderText -match "window\.visualViewport" -and $gameSizeProviderText -match "visualViewport\?\.addEventListener\('resize'" -and $gameSizeProviderText -match "visualViewport\?\.addEventListener\('scroll'" -and $gameSizeProviderText -match "handleOrientationChange" -and $gameSizeProviderText -match "setTimeout\(handleResize") "Mobile sizing must use the visual viewport, not only layout viewport, and must re-check after delayed mobile orientation updates"
...
Add-Check "mobile-playability-derived-state" ((Test-Path $mobilePlayabilityFile) -and $mobilePlayabilityText -match "getMobilePlayability" -and $mobilePlayabilityText -match "readMobilePointerState" -and $appText -match "getMobilePlayability" -and $appText -notmatch "orientation:portrait" -and $fullscreenGateText -match "needsFullscreenGate" -and $mobilePlayabilityTestText -match "mobile portrait always requires the rotate notice" -and $mobilePlayabilityTestText -match "does not deadlock behind the rotate notice" -and $mobilePlayabilityTestText -match "needsLandscapeNotice\)\.toBe\(false\)") "Mobile rotate/fullscreen gating must be derived from visual viewport dimensions and pointer state instead of a dismissible orientation flag, without trapping landscape mobile behind the rotate notice"
...
Add-Check "mobile-card-overflow-tests" ($cardLayoutMetricsText -match "getCardLayoutMetrics" -and $mobileCardLayoutTestText -match "visibleHandCounts" -and $mobileCardLayoutTestText -match "maxCardsInHand \+ 1" -and $mobileCardLayoutTestText -match "visibleHandCount - 1" -and $mobileCardLayoutTestText -match "toBeLessThanOrEqual\(\s*viewport\.width" -and $mobileCardLayoutTestText -match "toBeGreaterThanOrEqual\(rects\[i - 1\]\.right") "Mobile card layout must be tested for every visible hand count from one card through the historical max rendered slots"
