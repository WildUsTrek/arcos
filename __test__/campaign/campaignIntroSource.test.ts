...
const mobilePlayabilityPath = path.join(
  import.meta.dir,
  '../../src/utils/mobilePlayability.ts',
)
...
test('mobile layout uses visual viewport and requires fullscreen when supported', () => {
  ...
  const mobilePlayabilitySource = fs.readFileSync(mobilePlayabilityPath, 'utf8')
  ...
  expect(fullscreenGateSource).toContain('needsFullscreenGate')
  expect(mobilePlayabilitySource).toContain('pointer: coarse')
...
test('mobile card layout reserves side safe area', () => {
  ...
  expect(source).toContain('handCount <= maxCardsInHand + 1')
  expect(source).toContain('card-pos-${mode}')
  expect(source).toContain('--cardwidth')
